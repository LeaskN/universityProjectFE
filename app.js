...
window.addEventListener('load', () => {
      ...
      <form>
        ...
        <div class="form-group">
          <label for="poster_url">Image Link</label>
    ...
    `;
      ...
      const loadKeywords = keywords => {
        for (var i = 0; i < keywords.length; i++) {
          ...
          draggableEl.setAttribute('class', 'listItem');
    ...
    `;
        ...
        axios.get(baseURL)
          ...
          .then(universities => {
            allKeywords = universities.data.map(university => university.keywords).join(', ').split(', ');
                ...
                let positivePoints = preppingParaPos(positiveString)
                  ...
                  function preppingInputPos(string) {
                    ...
                    for (var i = 0; i < string.length; i++){
                      if (punctuation.indexOf(string[i]) === -1){
                        noPuncLowerCase += string[i].toLowerCase();
                      }
                    }

                    noPuncLowerCaseArr = noPuncLowerCase.split(' ');

                    for (var i = 0; i < noPuncLowerCaseArr.length; i++){
                      if (noPuncLowerCaseArr[i].length > 3){
